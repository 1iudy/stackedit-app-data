...
#### 训练结构判据
在输入文件中提前定义力不确定性阈值，初始阈值由**ML_CTIFOR**（单位 eV/Å）给出，其后续如何自适应由 `ML_ICRITERIA` 控制：`0` 不更新；`1` 用历史贝叶斯不确定性的均值更新；`2` 用滑动均值。推荐自动更新。`ML_ICRITERIA = 1`
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc2ODg5NzE5NCwtNjg0NDc0MDI2LDE3MT
